...
from TikTokLive.events import FollowEvent, GiftEvent, DisconnectEvent
...
def send_webhook(event, description, error_detail=None, color="242424"):
    ...
    try:
        ...
        if response.status_code != 200:
            log_message(f"Webhook failed with status code {response.status_code}: {response.text}")
    except Exception as e:
        log_message(f"Failed to send webhook: {e}")
...
class DeviceController:
    ...
    def play_video(self, video_path):
        try:
            ...
            log_message(f"Error playing video: {e}")
    ...
    def control_device(self, device_id, turn):
        ...
        with rate_limiter:
            try:
                ...
                log_message(f"Error controlling device: {err}")
...
ACTIONS = {
    ...
    5000: lambda count: log_message("5000")
}
...
async def on_gift(event: GiftEvent):
    ...
    log_message(f"Received a gift from {event.user.unique_id}: {event.gift.name} x{event.repeat_count}")
...
log_text = scrolledtext.ScrolledText(root, state='disabled', width=100, height=20)
